...
Конечно, концептуальное представление у разных людей может быть очень разное, но это не значит, что надо делать именно так, чтобы было понятно абсолютно всем, ведь если есть полноценные блэк-бокс тесты, то человек перенявший программу будет иметь возможность переделать её под собственное понимание.
...
Логика будет создана в объектно-ориентированном стиле как композиция классов через dependency injection. Это позволит нам отложить такие детали, как выбор хранилища или как будет происходить общение с сервисами и т.д.

Модель будет связана с представлением с помощью функции отображения, аналогичной ViewModel. У неё будет необходимая функциональность, чтобы иметь необходимое соответствие с вью, но не будет знания о деталях этого самого вью (например, о Реакте или DOMе и т.д.)

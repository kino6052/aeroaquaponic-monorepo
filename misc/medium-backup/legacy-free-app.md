...
Все было сведено к двум ключевым потребностям: мгновенная обратная связь и правильные шаблоны проектирования. Что касается шаблонов проектирования, то особое внимание было уделено жесткому разделению представления и логики.
...
- Детали 
  ...
  - Дизайн как единый источник правды для слоя представления 
- Процесс разработки 
  - От внешнего к внутреннему 
  - TDD 
  
Но хватит философии, перейдем к делу.

### Разработка

В этой статье я продемонстрирую процесс создания приложения в стиле "извне во внутрь" (outside-in), где конечным источником правды является дизайн в Figma.
...
Логика будет создана в объектно-ориентированном стиле как композиция классов через dependency injection. Это позволит нам отложить такие детали, как выбор хранилища или как будет происходить общение по HTTP и т.д.

Модель будет связана с представлением с помощью функции отображения, аналогичной ViewModel. Она будет иметь необходимую функциональность, чтобы иметь необходимое соответствие с вью, но не будет знать ничего о деталях этого самого вью (например, о Реакте или DOMе и т.д.)
...
Так как инструменты для конвертирования из дизайнов в код все еще далеки от идеала, нам следует рассчитывать больше на свои собственные силы при конвертировании компонентов в первый раз. Тем не менее, по мере того как будет меняться дизайн, мы можем попросить LLM (large language models как ChatGPT) адаптировать изменения в код компонентов, который уже существует. Такой подход гораздо проще, так как если компоненты реализованы "правильно", они обычно совсем небольшие и легко понятны для LLM.
...
По мере того как мы пишем тесты, мы реализуем логику домена.

Признаюсь, я разработал примерное приложение с очень небольшим количеством тестов и больше полагался на систему типов TypeScript для мгновенной обратной связи, поэтому в качестве личного TODO мне нужно будет убедиться, что я освою эту практику, так как я верю, что это в конечном итоге экономит много времени для крупных проектов, таких как этот.

Хотя наши тесты должны показывать, правильно ли работает функциональность, структура логики домена сама по себе не является вопросом правильности или неправильности. Концептуальная модель приложения - это описание того, как человек, написавший его, понимает программу концептуально, и лучше, если это будет один человек или группа, находящаяся на одной концептуальной волне.

Как небольшое философское отступление, хотелось бы отметить, что Иммануил Кант революционизировал философию, сместив фокус с идеи о том, что мы напрямую постигаем мир таким, каким он есть на самом деле, к идее о том, что мы постигаем мир таким, каким он является нам через наше сознание. Это означает, что, когда бы изучаем мир, то мы изучаем скорее то как он в нас отражается, нежели мир сам по себе.
...
Тем не менее, я верю, что каждый UI разработчик мечтал хотя бы раз в жизни о том, чтобы представить свое приложение в виде простых классов, которые читаются как повседневная речь.
...
Но к сожалению, чтобы рассмотреть пример потребуется отдельная статья.

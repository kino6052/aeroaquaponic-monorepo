import { StateService } from "./state.refactored.service";
import { FieldService as Field01Service } from "./field01.refactored.service";
import { FieldService as Field02Service } from "./field01.refactored.service";
import { FieldService as Field03Service } from "./field01.refactored.service";
import { Service } from "./service";

beforeEach(() => {
  SubmitService.resetInstance();
  Field01Service.resetInstance();
  Field02Service.resetInstance();
  Field03Service.resetInstance();
  SubmitIntegrationService.resetInstance();
});

describe("Extra Tests", () => {
  it("should not create instance when there is one already", () => {
    expect(SubmitService.getInstance()).toEqual(SubmitService.getInstance());
    expect(SubmitIntegrationService.getInstance()).toEqual(
      SubmitIntegrationService.getInstance()
    );
    expect(
      FieldIntegrationService.getInstance(
        FieldService.getInstance(),
        StateService.getInstance()
      )
    ).toEqual(
      FieldIntegrationService.getInstance(
        FieldService.getInstance(),
        StateService.getInstance()
      )
    );
  });
});

describe("Submit Integration", () => {
  it("should call submit on click", () => {
    SubmitIntegrationService.getInstance();
    const submitService = SubmitService.getInstance();
    const spy = jest
      .spyOn(submitService, "submit")
      .mockImplementation(() => undefined);
    Service.EventSubject.next(["change", submitService.id, "123"]);
    expect(spy).toHaveBeenCalledTimes(1);
  });
});

describe("Submit", () => {
  it("should submit", async (done) => {
    SubmitIntegrationService.getInstance();
    const submitService = SubmitService.getInstance();
    const filed01Service = Field01Service.getInstance();
    const spy01 = jest.spyOn(field01Service, "validate");
    const filed02Service = Field02Service.getInstance();
    const spy02 = jest.spyOn(field02Service, "validate");
    const filed03Service = Field03Service.getInstance();
    const spy03 = jest.spyOn(field03Service, "validate");
    await submitService.submit();
    expect(spy01).toHaveBeenCalledTimes(1);
    expect(spy02).toHaveBeenCalledTimes(1);
    expect(spy03).toHaveBeenCalledTimes(1);
    done();
  });
});
